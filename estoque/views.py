...
from .models import Produto, Especie, Fornecedor, Entrada, ItemEntrada, MotivoBaixa, Saida, ItemSaida, Baixa, ItemBaixa, Usuario, HistoricoEntrada, HistoricoSaida, HistoricoBaixa, AuditoriaExclusao, ItemAuditoriaExclusao, Empresa, Estoque
from .forms import ProdutoForm, EspecieForm, FornecedorForm, EntradaForm, ItemEntradaFormSet, MotivoBaixaForm, SaidaForm, ItemSaidaFormSet, BaixaForm, ItemBaixaFormSet, UsuarioForm, MudarSenhaForm, EmpresaForm
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and formset.is_valid():
            with transaction.atomic():
                ...
                itens = formset.save()
                for p in set(i.produto for i in itens): p.atualizar_estoque(empresa)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and formset.is_valid():
            with transaction.atomic():
                ...
                afetados = set(i.produto for i in entrada.itens.all()).union(set(i.produto for i in itens_salvos))
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and formset.is_valid():
            with transaction.atomic():
                ...
                afetados = set(i.produto for i in saida.itens.all()).union(set(i.produto for i in itens_salvos))
                for p in afetados: p.atualizar_estoque(empresa)
            return redirect('saida_list')
    else:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and formset.is_valid():
            with transaction.atomic():
                ...
                afetados = set(i.produto for i in baixa.itens.all()).union(set(i.produto for i in itens_salvos))
                for p in afetados: p.atualizar_estoque(empresa)
            return redirect('baixa_list')
    else:
...
def relatorio_kardex(request):
    ...
    if produto_id and context['data_inicial'] and context['data_final']:
        ...
        saldo = in_ant - out_ant - loss_ant

        movs = []
        for e in ItemEntrada.objects.filter(produto=produto_selecionado, entrada__empresa_id=empresa_id, entrada__data_entrada__range=(d_ini, d_fim)): movs.append({'data': e.entrada.data_entrada, 'tipo': 'Entrada (NF)', 'documento': f"NF: {e.entrada.nota_fiscal}", 'lote': e.lote, 'entrada': e.quantidade, 'saida': 0, 'usuario': e.entrada.usuario_registro.username})
        for s in ItemSaida.objects.filter(produto=produto_selecionado, saida__empresa_id=empresa_id, saida__data_saida__range=(d_ini, d_fim)): movs.append({'data': s.saida.data_saida, 'tipo': 'Saída Paciente', 'documento': f"Pac: {s.saida.paciente}", 'lote': s.lote, 'entrada': 0, 'saida': s.quantidade, 'usuario': s.saida.usuario_registro.username})
        for b in ItemBaixa.objects.filter(produto=produto_selecionado, baixa__empresa_id=empresa_id, baixa__data_baixa__range=(d_ini, d_fim)): movs.append({'data': b.baixa.data_baixa, 'tipo': 'Descarte', 'documento': b.baixa.motivo.nome, 'lote': b.lote, 'entrada': 0, 'saida': b.quantidade, 'usuario': b.baixa.usuario_registro.username})
        ...
        context.update({'movimentacoes': movs, 'saldo_anterior': in_ant - out_ant - loss_ant, 'produto_selecionado': produto_selecionado, 'data_inicial_formatada': d_ini.strftime('%d/%m/%Y'), 'data_final_formatada': d_fim.strftime('%d/%m/%Y')})
